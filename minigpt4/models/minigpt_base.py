...
class MiniGPTBase(BaseModel):
        ...
        if prompts is None or len(prompts) == 0:
            ...
            for idx, (each_img_embed, each_prompt) in enumerate(zip(img_embeds, prompts)):
                ...
                for idx, seg in enumerate(p_segs[:-1]):
                    ...
                    input_ids.append(p_tokens.input_ids[0]) 
            ...
            return wrapped_embs, wrapped_atts, input_ids 
    ...
    def preparing_embedding(self, samples):
        ...
        if 'conv_q' in samples:
            ...
            cond_embeds, cond_atts, input_ids = self.prompt_wrap(img_embeds, img_atts, [q[0] for q in conv_q])
            ...
            if 'length' in samples:
                ...
                cond_embeds, cond_atts, input_ids = self.prompt_wrap(img_embeds, img_atts, instruction, samples['length'])
            else:
                cond_embeds, cond_atts, input_ids = self.prompt_wrap(img_embeds, img_atts, instruction)
        ...
        return cond_embeds, cond_atts, regress_embeds, regress_atts, part_targets, input_ids 

    def forward(self, samples, reduction='mean'):

        # prepare the embedding to condition and the embedding to regress
        cond_embeds, cond_atts, regress_embeds, regress_atts, part_targets, input_ids = \
        ...
        return {"loss": outputs.loss, "logits": outputs.logits, "targets": targets, "input_ids": input_ids}
        # return outputs.loss, outputs.logits, targets
        # return {"loss": loss}
